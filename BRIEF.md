...
Clarification after independent review, 25 September: artifact directories may be under the requested output parent, but each executable consumer must be newly allocated outside the source checkout and must not resolve packages through ancestor node_modules. This reconciles the original output-location suggestion with the frozen evaluation protocol. Verify actual resolution and installed bytes; a temporary directory alone is not evidence of isolation. Never delete user files or unrelated directories. Make cleanup optional initially; keep failures inspectable. No network dependencies in fixture packages. Escape output when later presenting report content in HTML.
